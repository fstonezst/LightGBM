...
public:
  ...
  void ConstructHistogram(int leaf, const GradHessPair* gpair, int num_bin,
    ...
    for (; i < end - group_rest;) {
      ...
      for (data_size_t j = 0; j < KNumSumupGroup; j += 8, i += 8) {
        ...
        AddPairToHistogram(tmp_sumup_buf, bin0, bin1, bin2, bin3, bin4, bin5, bin6, bin7,
                           gpair[ordered_pair_[i].ridx],
                           gpair[ordered_pair_[i + 1].ridx],
                           gpair[ordered_pair_[i + 2].ridx],
                           gpair[ordered_pair_[i + 3].ridx],
                           gpair[ordered_pair_[i + 4].ridx],
                           gpair[ordered_pair_[i + 5].ridx],
                           gpair[ordered_pair_[i + 6].ridx],
                           gpair[ordered_pair_[i + 7].ridx]);
    ...
    for (; i < end - rest; i += 8) {
      ...
      AddPairToHistogram(out, bin0, bin1, bin2, bin3, bin4, bin5, bin6, bin7,
                         gpair[ordered_pair_[i].ridx],
                         gpair[ordered_pair_[i + 1].ridx],
                         gpair[ordered_pair_[i + 2].ridx],
                         gpair[ordered_pair_[i + 3].ridx],
    ...
    for (; i < end; ++i) {
      const VAL_T bin0 = ordered_pair_[i].bin;
      out[bin0].sum_gradients += gpair[ordered_pair_[i].ridx].grad;
      out[bin0].sum_hessians += gpair[ordered_pair_[i].ridx].hess;
      ++out[bin0].cnt;
    }
  }
